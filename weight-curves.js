...
const WeightCurves=(()=>{
 ...
 function init(api){
  ...
  function refresh(){const s=api.settings();if(active&&active.key!==key(active.i,s)){api.cancel();active=null;}for(let i=0;i<17;i++)if(!show(i))placeholder(i,'화면에 보이면 새 설정으로 계산합니다.');schedule();}
