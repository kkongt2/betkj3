...
const WeightSearchEngine=(()=>{
 ...
 async function run(options,fast,verify,control={}){
  const now=control.now||(()=>performance.now()),random=control.random||Math.random,current=control.current||(()=>true),stopped=control.stopped||(()=>false),progress=control.progress||(()=>{});
