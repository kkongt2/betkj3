...
const settings={...T.settings(),anchorMode:'analysis',anchorRank:3,min:2,max:6};
...
(async()=>{
 ...
 if(process.argv.includes('--archive')){
  ...
  for(const anchorMode of ['odds','analysis'])for(const anchorRank of [1,2,3]){
   const config={...seed,anchorMode,anchorRank};const a=await fast.evaluate(config,'00000000','99999999'),b=await exact.evaluate(config,'00000000','99999999');assert.deepEqual(a.all,b.all);console.log('PASS exact archive statistics',anchorMode,anchorRank,rows.length,a.all.hits,a.all.evaluated);
  }
