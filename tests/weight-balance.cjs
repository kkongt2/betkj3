...
for(const base of bases){const w=b.project([...base.settings.weights,0]);assert(b.valid(w));assert.equal(w.reduce((a,b)=>a+b,0),100);assert(w.every(x=>x>=3&&x<=20));const invalid=w.slice();invalid[0]=2;assert(!b.valid(invalid));assert(!b.valid([...base.settings.weights,0]));}
const w=b.project(t.defaults());const field=Array.from({length:8},(_,i)=>[i+1,.3,1,[],null,null,Array.from({length:17},(_,j)=>(i+j)%7/6)]),pairs=[];for(let a=1;a<=8;a++)for(let c=a+1;c<=8;c++)pairs.push([a,c,.1]);
