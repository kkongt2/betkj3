...
(async()=>{
 ...
 try{
  ...
  await page.selectOption('#weightSearchMethod','de');await page.locator('#weightSearchSeconds').fill('2');await page.locator('#weightSearchBalanced').uncheck();
  const before=await page.evaluate(()=>strategySettings());
  await page.locator('#prepareRunnerSearch').click();await page.locator('#runnerSearchRequest').waitFor();const payload=JSON.parse(await page.locator('#runnerSearchRequest').inputValue());assert.equal(payload.seconds,2);assert.equal(payload.method,'de');assert.deepEqual(payload.settings.weights,before.weights);
  ...
  assert((await page.locator('#runnerSearchResults').innerText()).includes('내 최근 요청'));
  ...
  await page.locator('#prepareRunnerSearch').click();await page.locator('#weightSearchSeconds').fill('3');await page.locator('#weightSearchSeconds').blur();assert.equal(await page.locator('#runnerPrepared').isHidden(),true);
