...
let total=0;for(const f of r.folds){assert(r.search.scaleFitThrough<f.validationFrom);assert(f.trainThrough<f.validationFrom);assert(f.validationThrough<f.from);assert(f.through<f.to);assert(covers(f.selected));assert.deepEqual(f.settings,p.config(f.settings));total+=f.selected.total;
