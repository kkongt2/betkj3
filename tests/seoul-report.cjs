const fs=require('node:fs'),assert=require('node:assert/strict'),t=require('../tuning-model.js'),p=require('../strategy-presets.js'),{covers,evaluate}=require('../scripts/rolling-search.cjs');
const r=JSON.parse(fs.readFileSync('rolling-report.json'));assert.equal(r.scope,'seoul');assert.equal(r.version,t.VERSION);assert(r.folds.length>=4);assert.equal(r.minimumCoverageRatio,.4);assert(!r.regional&&!r.common);
...
let total=0;for(const f of r.folds){assert(r.search.scaleFitThrough<f.validationFrom);assert(f.trainThrough<f.validationFrom);assert(f.validationThrough<f.from);assert(f.through<f.to);assert(covers(f.selected));assert.deepEqual({anchorRank:1,...f.settings},p.config(f.settings));total+=f.selected.total;
