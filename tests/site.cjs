...
vm.createContext(sandbox);for(const p of ['model.js','qpl-policy.js','tuning-model.js','qpl-history-engine.js','strategy-presets.js','rolling-panel.js','top5-panel.js','app.js'])vm.runInContext(fs.readFileSync(p,'utf8'),sandbox);
(async()=>{await flush();
 ...
 const top5=JSON.parse(fs.readFileSync('top5-presets.json'));assert(nodes['#top5Presets'].innerHTML.includes('16개 가중치 보기'));for(let i=0;i<5;i++){nodes['#top5Presets'].click({target:{closest:()=>({dataset:{top5:String(i)}})}});await flush();assert.equal(vm.runInContext('tuningSettings.weights.join(",")',sandbox),top5.presets[i].settings.weights.join(','));assert.equal(vm.runInContext('partnerRange.min',sandbox),top5.presets[i].settings.min);assert(nodes['#top5Status'].textContent.includes('저장했습니다'));}
