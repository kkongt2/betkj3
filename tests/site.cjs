...
vm.createContext(sandbox);for(const p of ['model.js','qpl-policy.js','tuning-model.js','qpl-history-engine.js','strategy-presets.js','rolling-panel.js','app.js'])vm.runInContext(fs.readFileSync(p,'utf8'),sandbox);
(async()=>{await flush();
