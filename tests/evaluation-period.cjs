...
const fixture=(date,hit=true)=>({venue:'seoul',date,race:1,k:2,models:{place:'test',pair:'test'},horses:Array.from({length:6},(_,i)=>[i+1,.3,1,[],null,null,Array(17).fill(.5),{starts:i===0?0:5,distanceStarts:i===0?0:3,recordStarts:i===0?0:3,marginStarts:i===0?0:5,available:Array(17).fill(i!==0)}]),pairs:Array.from({length:6},(_,i)=>Array.from({length:5-i},(_,j)=>[i+1,i+j+2,.1])).flat(),quotes:Array.from({length:6},(_,i)=>({numbers:[i+1],odds:1+i/10})),starters:[1,2,3,4,5,6],settled:true,payouts:[{numbers:hit?[1,2]:[5,6],odds:hit?4:100}]});
(async()=>{
 const settings={...T.settings(),anchorMode:'odds',anchorRank:1,min:2,max:2},rows=[fixture('20211231'),fixture('20220101'),fixture('20221231',false),fixture('20230101'),fixture('20270101')],engine=H.create(rows),to='20260913';
