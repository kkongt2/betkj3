...
(async()=>{
 ...
 try{
  ...
  assert.equal(await page.locator('#weightSearchParallel').isChecked(),true);assert.equal(await page.locator('#weightSearchSeconds').getAttribute('max'),'3600');
  await page.locator('#weightSearchSeconds').fill('3601');await page.locator('#startWeightSearch').click();assert((await page.locator('#weightSearchStatus').innerText()).includes('1~3600'));
  for(const method of ['local','de'])for(const mode of ['all','joint']){
   await page.selectOption('#weightSearchMethod',method);await page.selectOption('#weightSearchMode',mode);await page.locator('#weightSearchSeconds').fill('2');
   ...
   const status=await page.locator('#weightSearchStatus').innerText();assert(status.includes('병렬 3개 작업'),status);assert(status.includes('검산 완료'));console.log(method,mode,status);
  ...
  await page.locator('#weightSearchSeconds').fill('3600');await page.locator('#startWeightSearch').click();
  await page.waitForFunction(()=>document.querySelector('#weightSearchStatus').textContent.includes('/ 3600초'));
  await page.locator('#stopWeightSearch').click();await page.waitForFunction(()=>!document.querySelector('#startWeightSearch').disabled,{},{timeout:30000});assert((await page.locator('#weightSearchStatus').innerText()).includes('중지 완료'));
  ...
  await page.locator('#weightSearchSeconds').fill('1');await page.locator('#startWeightSearch').click();await page.waitForFunction(()=>!document.querySelector('#applyWeightSearch').disabled,{},{timeout:60000});assert((await page.locator('#weightSearchStatus').innerText()).includes('단일 작업'));
  ...
  await context.close();assert.deepEqual(errors,[]);console.log('PASS mobile parallel local/DE all/joint, exact parity, 3600-second start/stop, option persistence, cancellation, worker-failure fallback and screening OFF');
