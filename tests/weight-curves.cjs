...
function fixture(n,race){const horses=Array.from({length:n},(_,i)=>[i+1,.3,1,[],null,null,Array.from({length:17},()=>rand())]);return {venue:'seoul',date:'20250101',race,k:n<=7?2:3,horses,pairs:horses.flatMap((a,i)=>horses.slice(i+1).map(b=>[a[0],b[0],.1])),quotes:horses.map(a=>({numbers:[a[0]],odds:1+Math.floor(rand()*6)/10})),starters:horses.map(a=>a[0]),settled:true,payouts:[{numbers:[1,2],odds:3.4},{numbers:[2,3],odds:5.2},{numbers:[1,3],odds:2.1}]};}
function direct(rows,settings,from='00000000',to='99999999'){
 ...
 for(const row of rows){if(row.venue!=='seoul'||row.date<from||row.date>to)continue;g.total++;if(!row.settled){g.excluded++;continue;}
  const result=p.apply(t.apply(t.unpack(row),settings),{quotes:row.quotes},settings);if(result.qplPolicy.status!=='ready'){g.excluded++;continue;}g.evaluated++;
...
(async()=>{
 ...
 console.log('PASS all 101 points against exact model: both anchors, ties, tiny gaps, uniform strengths, scratched runners, missing/duplicate quotes, unavailable pairs, date/venue exclusions, zero total and cancellation');
