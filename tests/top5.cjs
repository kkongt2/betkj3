...
for(const preset of report.presets){
 ...
 assert.equal(preset.settings.anchorMode,base.settings.anchorMode);assert(Math.abs(preset.settings.min-base.settings.min)<=3);assert(Math.abs(preset.settings.max-base.settings.max)<=3);assert(weightMove(preset.settings.weights,base.settings.weights)<=30);
...
console.log('PASS 15 exact last-five presets: 5 descendants per base 3/4/9, 17 integer weights, 15% hits, 40% coverage, 10% pick differences and bounded weight/rank changes');
