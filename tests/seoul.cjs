...
const a=Array(16).fill(0),b=Array(16).fill(0);a[0]=100;b[1]=100;
const old={anchorMode:'analysis',min:2,max:6,modelMode:'custom',weights:a,weightScope:'venue',venueWeights:{seoul:b,busan:a,jeju:a}};
assert.deepEqual(t.settings(old).weights,b);assert(!('venueWeights' in t.settings(old)));
assert.equal(t.settings({modelMode:'existing'}).modelMode,'custom');assert.equal(t.settings({weights:Array(13).fill(1)}).weights.length,16);
...
const race={venue:'seoul',date:'20250101',race_no:1,horses:Array.from({length:8},(_,i)=>({number:i+1,name:'H'+i,weighted_v3_features:Array.from({length:16},(_,j)=>j===0?i/7:j===1?1-i/7:.5)})),official_result:{starters:[1,2,3,4,5,6,7,8],pair:{status:'confirmed',payouts:[{numbers:[1,8],odds:4}]}}};
const base=m.analyze(race),quotes={quotes:race.horses.map(h=>({numbers:[h.number],odds:1+h.number/10}))};
...
(async()=>{const g=await evaluator.evaluate({...old,min:2,max:8},'00000000','99999999');assert.equal(g.all.total,1);assert.deepEqual(Object.keys(g),['all','seoul']);
 ...
 for(const f of fs.readdirSync('data/market-odds'))assert(Object.keys(JSON.parse(fs.readFileSync('data/market-odds/'+f)).races).every(k=>k.startsWith('seoul:')));
 const scale=JSON.parse(fs.readFileSync('data/weighted-v3-scaler.json'));assert.equal(scale.scope,'seoul');assert.equal(scale.version,'weighted-seoul-v1');
 console.log('PASS Seoul-only data, calendar, prices, features, denominator, prediction guard, weight changes and saved-settings migration:',n,'races');
