...
const rows=manifest.shards.flatMap(s=>JSON.parse(fs.readFileSync(s.url)).rows).filter(r=>r.date<='20231231');
...
for(const r of valid){
 const hs=r.horses.filter(h=>!r.starters||r.starters.includes(h[0])).sort((a,b)=>a[0]-b[0]);
 lines.push([r.date,hs.length,r.k].join(' '));
 for(const h of hs)lines.push([h[0],(!r.starters||r.starters.includes(h[0]))?r.quotes.find(q=>q.numbers[0]===h[0]).odds:-1,h[1],...h[4]].join(' '));
