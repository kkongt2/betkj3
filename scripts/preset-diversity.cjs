...
const rangeKey=c=>[c.settings.anchorMode,c.settings.min,c.settings.max].join(':');
const bandKey=c=>c.settings.anchorMode+':'+(c.settings.min<=4?'2-4':c.settings.min<=7?'5-7':c.settings.min<=10?'8-10':'11-20');
function selectDiverse(candidates,rules=RULES){
 ...
 while(selected.length<rules.count){
  ...
  for(const other of pool){if(other.chosen)continue;const d=pairDistance(other.predictions,c.predictions),w=weightDistance(other.settings.weights,c.settings.weights),a=other.settings.anchorMode===c.settings.anchorMode?0:1;other.minPairDistance=Math.min(other.minPairDistance,d);other.minDiversity=Math.min(other.minDiversity,.7*d+.2*w+.1*a);}
 }
