...
function exportTrain(rows,file){const valid=rows.filter(r=>r.settled&&r.horses.filter(h=>!r.starters||r.starters.includes(h[0])).length>=3);const lines=[valid.length+' '+rows.length];for(const r of valid){const hs=r.horses.filter(h=>!r.starters||r.starters.includes(h[0])).slice().sort((a,b)=>a[0]-b[0]);lines.push([r.date,hs.length,r.k].join(' '));for(const h of hs){assert.equal(h[6]?.length,17);lines.push([h[0],0,h[1],...h[6]].join(' '));}for(const a of hs)for(const b of hs){const p=a!==b?r.payouts.find(x=>x.numbers.includes(a[0])&&x.numbers.includes(b[0])):null;lines.push('0 '+(p?.odds||0));}}fs.writeFileSync(file,lines.join('\n'));}
