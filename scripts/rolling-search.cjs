...
async function main(){
 const manifest=JSON.parse(fs.readFileSync('qpl-history.json')),rows=manifest.shards.flatMap(s=>JSON.parse(fs.readFileSync(s.url)).rows),tmp=fs.mkdtempSync(path.join(os.tmpdir(),'rolling-v3-'));
 ...
 function search(train,style){assert(train.every(r=>r.date<activeValidation));const file=path.join(tmp,'train.txt'),out=path.join(tmp,'candidates.json');exportTrain(train,file);cp.execFileSync('/tmp/search-v3',[file,out,String(style)],{stdio:['ignore','pipe','inherit']});const d=JSON.parse(fs.readFileSync(out));tested+=d.weightCandidates;return d.finalists.map(x=>x.weights);}
 ...
 for(let from='20240101';from<=lastQuarter;from=shift(from,3)){
  ...
  for(let style=0;style<4;style++){
   const pool=[];for(const months of [24,36]){const trainFrom=shift(from,-months),train=slice(trainFrom,validationFrom);assert(train.length>=500);const weights=search(train,style);
    for(const w of weights){const candidates=run(validation,w).filter(r=>covers(r.all));candidates.sort(compare);if(candidates[0])pool.push({...candidates[0],months,style,trainFrom,trainThrough:train.at(-1).date});}
   }
 ...
 const report={schema:1,version:tuning.VERSION,generatedAt:new Date().toISOString(),sourceRaces:rows.length,from:folds[0].from,to:manifest.to,minimumCoverageRatio:.4,search:{weightCandidates:tested,windows:[24,36],innerSelectionMonths:3,assessmentMonths:3,scaleFitThrough:scaler.fitThrough,venuePriorRaces:1000,minimumVenueTrain:500,minimumVenueSelection:100},folds,common:aggregate(commonResults),regional:aggregate(regionalResults),selected:aggregate(selectedResults),recordComparison:STYLES.map((label,i)=>({label,...aggregate(styleResults[i])})),live:{...live,retrospective:{all:retrospective.all,metrics:retrospective.metrics}},sourceSha256:Object.fromEntries(manifest.shards.map(s=>[s.url,crypto.createHash('sha256').update(fs.readFileSync(s.url)).digest('hex')])),note:'각 분기 직전 3개월로 가중치·학습기간·기록 방식·공통/경마장별 사용 여부를 선택하고 다음 분기를 평가했습니다. 각 분기의 전체 경주 40% 이상 조건을 확인합니다. 과거 자료는 이전 실험에도 사용됐으므로 완전히 새로운 독립 검증은 아닙니다. 최종배당 기준 사후 재계산입니다. 진행 중 분기는 확보된 날짜까지만 집계합니다. 현재 선택 조합의 전체 과거 재계산과 분기별 고정 조합 성적은 서로 다릅니다.'};
...
module.exports={shift,quarter,normalize,blend,evaluate,aggregate,covers,drawdown};
