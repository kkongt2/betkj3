...
function evaluate(rows,config){rows=rows.filter(r=>r.venue==='seoul');const g=blank(),days={};for(const row of rows){g.total++;if(!row.settled){g.excluded++;continue;}const b=tuning.apply(tuning.unpack(row),config),r=policy.apply(b,{quotes:row.quotes},config);if(r.qplPolicy.status!=='ready'){g.excluded++;continue;}const ns=r.pairs[0].numbers.slice().sort((a,b)=>a-b).join('-'),paid=row.payouts.find(p=>p.numbers.slice().sort((a,b)=>a-b).join('-')===ns)?.odds||0;g.evaluated++;g.hits+=paid>0;g.paidHits+=paid>0;g.payoutTotal+=paid;days[row.date]=(days[row.date]||0)+paid-1;}return {all:g,days,metrics:engine.metrics(g)};}
...
function exportTrain(rows,file){const valid=rows.filter(r=>r.settled&&r.horses.filter(h=>!r.starters||r.starters.includes(h[0])).length>=3&&r.quotes.every(q=>q.numbers.length===1&&Number.isFinite(q.odds)&&q.odds>=1)&&new Set(r.quotes.map(q=>q.numbers[0])).size===r.quotes.length&&r.horses.filter(h=>!r.starters||r.starters.includes(h[0])).every(h=>r.quotes.some(q=>q.numbers[0]===h[0])));const lines=[valid.length+' '+rows.length];for(const r of valid){const hs=r.horses.filter(h=>!r.starters||r.starters.includes(h[0])).slice().sort((a,b)=>a[0]-b[0]);lines.push([r.date,hs.length,r.k].join(' '));for(const h of hs){assert.equal(h[6]?.length,17);lines.push([h[0],r.quotes.find(q=>q.numbers[0]===h[0]).odds,h[1],...h[6]].join(' '));}for(const a of hs)for(const b of hs){const p=a!==b?r.payouts.find(x=>x.numbers.includes(a[0])&&x.numbers.includes(b[0])):null;lines.push('0 '+(p?.odds||0));}}fs.writeFileSync(file,lines.join('\n'));}
