...
function run(rows,weights,options={}){
 ...
 const results=[];for(const anchorMode of ['odds','analysis'])for(let min=2;min<=20;min++)for(let max=min;max<=20;max++){if(options.family&&(anchorMode!==options.family.anchorMode||Math.abs(min-options.family.min)>3||Math.abs(max-options.family.max)>3))continue;results.push({settings:{anchorMode,modelMode:weights?'custom':'existing',weights:weights||tuning.defaults(),min,max},all:blank(),years:{},...(options.predictions?{predictions:new Uint16Array(rows.length)}:{})});}
 ...
 for(const [rowIndex,row] of rows.entries()){
  ...
  for(const anchorMode of ['odds','analysis']){
   if(options.family&&anchorMode!==options.family.anchorMode)continue;
   ...
   for(let min=2;min<=20;min++){let picked=null;for(let max=min;max<=20;max++){
    ...
    const out=grids[anchorMode][min]?.[max],year=row.date.slice(0,4);if(!out)continue;
