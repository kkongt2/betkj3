...
for(const base of bases){
 fs.writeFileSync(seed,[base.settings.anchorMode==='analysis'?1:0,base.settings.min,base.settings.max,...base.settings.weights].join(' '));
 ...
 const weights=[...new Map([base.settings.weights,...native.finalists.map(f=>f.weights)].map(w=>[w.join(','),w])).values()];
 baselines.push({id:base.id,settings:base.settings,...evaluate(rows,base.settings)});
 for(const w of weights){assert(weightMove(w,base.settings.weights)<=30);const results=run(rows,w,{predictions:true});tried+=results.length;exact++;
...
for(const candidate of selected){
 ...
 presets.push({rank:presets.length+1,name:'서울 5경주 기준'+candidate.baseId+' 파생 '+(presets.length+1)+'번',...saved,coverage:candidate.all.evaluated/rows.length});
}
assert.equal(presets.length,15);
const report={schema:3,selectionRules:RULES,bases:baselines,baseSource:parents.sourceCommit,bestEligibleProduct:board[0].metrics.product,scope:'seoul',version:t.VERSION,generatedAt:new Date().toISOString(),from:rows[0].date,to:rows.at(-1).date,sourceRaces:rows.length,minimumCoverageRatio:.4,minimumEvaluated:minimum,search:{nativeWeightCandidates:tested,exactWeightCandidates:exact,exactSettings:tried,anchors:['odds','analysis'],ranks:[2,20],weightStep:1,weightTotal:100,deduplicate:'identical historical selected pairs',eligibleDistinct:distinct.length},presets,sourceSha256:Object.fromEntries(manifest.shards.map(s=>[s.url,crypto.createHash('sha256').update(fs.readFileSync(s.url)).digest('hex')])),note:'기존 프리셋 3·4·9번을 새 최근 5경주 지표로 다시 계산하고 각 계열에서 5개씩 총 15개를 선별했습니다. 축마 방식은 기준과 같게 유지하며 순위 양 끝은 각각 ±3위, 가중치는 총 30%p 이내에서 재배분했습니다. 적중률 15% 이상과 전체 서울 경주 40% 이상 조건을 지키고 곱이 높은 후보부터 선택합니다. 실제 선택 조합 차이는 최소 10%, 같은 계열·순위 범위는 최대 2개입니다. 같은 과거 자료로 탐색·평가한 사후 성적이며 미래 성적이나 독립 검증이 아닙니다.'};
