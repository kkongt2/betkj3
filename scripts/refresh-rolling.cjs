...
const manifest=JSON.parse(fs.readFileSync('qpl-history.json')),d=manifest.to,q=d.slice(0,4)+String(Math.floor((+d.slice(4,6)-1)/3)*3+1).padStart(2,'0')+'01';
const report=fs.existsSync('rolling-report.json')?JSON.parse(fs.readFileSync('rolling-report.json')):null;
if(report?.version===require('../tuning-model.js').VERSION&&report.live?.forQuarter===q)console.log('Retain frozen quarterly settings:',q);
