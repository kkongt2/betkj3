...
int main(int argc,char**argv){
 ...
 struct Group{int min,max;std::vector<int> ids;};std::vector<Group> groups(ng);int member[17];
 ...
 auto valid=[&](const W&w){if(std::accumulate(w.begin(),w.end(),0)!=100)return false;for(int x:w)if(x<minWeight||x>maxWeight)return false;for(int gi=0;gi<ng;gi++){int sum=groupSum(w,gi);if(sum<groups[gi].min||sum>groups[gi].max)return false;}return true;};
 ...
 for(int a=0;a<17;a++)for(int b=0;b<17;b++)for(int shift:{1,3,5,10,15})if(a!=b&&origin[a]-shift>=minWeight){W w=origin;w[a]-=shift;w[b]+=shift;initial.push_back(w);}
 for(int trial=0;trial<1000;trial++){
  W w;w.fill(minWeight);double priority[17];for(double&v:priority){double u=1+(rng()%1000);v=u*u;}
  auto add=[&](const std::vector<int>& ids){int best=-1;double val=-1;for(int i:ids)if(w[i]<maxWeight&&groupSum(w,member[i])<groups[member[i]].max){double p=priority[i]/(w[i]-minWeight+1);if(p>val){val=p;best=i;}}if(best<0)return false;w[best]++;return true;};
  for(int gi=0;gi<ng;gi++)while(groupSum(w,gi)<groups[gi].min)if(!add(groups[gi].ids))return 5;
  std::vector<int> ids(17);std::iota(ids.begin(),ids.end(),0);while(std::accumulate(w.begin(),w.end(),0)<100)if(!add(ids))return 5;
  initial.push_back(w);
 }
 batch(initial);if(board.empty())return 4;
 for(int round=0;round<2;round++)for(int step:{3,1}){std::vector<W> next;auto top=board;for(int t=0;t<std::min(8,int(top.size()));t++)for(int a=0;a<17;a++)for(int b=0;b<17;b++)if(a!=b&&top[t].w[a]-step>=minWeight){W w=top[t].w;w[a]-=step;w[b]+=step;next.push_back(w);}batch(next);}
