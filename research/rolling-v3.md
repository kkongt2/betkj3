...
For each quarter starting in 2024, consider the preceding 24 and 36 months. The last three months within that window are the selection period; the preceding 21 or 33 months are used for candidate generation. Four record variants are compared: mean, median, best, and a blend including consistency. Candidate generation uses deterministic native search, integer 5/1-point transfers, both anchors and rank intervals. Finalists are recalculated with the site's exact JavaScript probability engine on the selection quarter.
