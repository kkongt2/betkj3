...
async function loadQplHistory(){
 ...
 try{
  const doc=await fetchJSON('qpl-history.json?v=6.0'+Date.now());
...
function setupHistoryEngine(){
 ...
 if(typeof Worker==='function')try{
  historyWorker=new Worker('qpl-history-worker.js?v=6.0');
...
function initTuning(){
 ...
 TuningModel.FEATURES.forEach((f,i)=>{
  ...
  for(const id of ['#weight-'+i,'#weight-range-'+i]){const node=$(id);node.oninput=()=>change(node);node.onchange=()=>{if(!change(node))node.value=String(tuningSettings.weights[i]??0);};}
 });
 $('#resetWeights').onclick=()=>{tuningSettings.weights=TuningModel.defaults();tuningSettings.modelMode='custom';update();};syncTuningControls=sync;sync();
}
...
function initStrategyPresets(){
 ...
 fetchJSON('strategy-search.json?v=6.0').then(doc=>{
