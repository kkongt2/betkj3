...
function analyzeForSite(r,mode='accuracy',inputOdds={place:{},qpl:{}}){
 return Betkj3Policy.apply(analyze(r,mode,inputOdds),loadedMarketOdds.get(r.date)?.races?.[r.venue+':'+r.race_no]?.place);
}
...
function pickHTML(x,r,type,lead=false){
 ...
 const explanation=type==='pair'?'연승 최종배당 1위와 배당 3·4위 중 기존 모델 동반입상확률이 높은 조합':reasons.length?reasons.join(' · '):'전체 경주용 모델의 1순위 후보';
...
function render(){if(!current)return;ranked=analyzeForSite(current,$('#mode').value,odds);const r=ranked;$('#analysis').hidden=false;$('#raceTitle').textContent=(names[r.venue]||r.venue_name||'직접 입력')+' '+r.race_no+'R';$('#raceMeta').textContent=[r.date,r.title,manual?'직접 입력 / 데모':''].filter(Boolean).join(' · ');$('#horseCount').textContent=r.horses.length+'두';$('#placeRule').textContent=r.k+'착 이내';$('#modeHelp').textContent=r.mode==='accuracy'?'연승은 기존 추정확률 순입니다. 복연승은 연승 최종배당 1위 + 배당 3·4위 중 기존 모델 우위 조합으로 고정합니다.':'연승은 입력 배당의 기대수익률 순입니다. 복연승 선정은 연승 최종배당 기준으로 유지됩니다.';const warning=timingReasons(r);$('#raceWarning').hidden=!warning.length;$('#raceWarning').textContent=warning.join(' · ');$('#placeLead').innerHTML=pickHTML(r.places[0],r,'place',true);$('#pairLead').innerHTML=pickHTML(r.pairs[0],r,'pair',true);$('#placeList').innerHTML=r.places.slice(1,5).map(x=>pickHTML(x,r,'place')).join('');$('#pairList').innerHTML='<p class="hint">복연승은 새 기준을 통과한 한 조합만 표시합니다.</p>';$('#horses').innerHTML=r.horses.map(h=>`<div class="horse"><strong>${h.number} ${esc(h.name)} · ${probabilityHTML(h.prob)}</strong><p class="hint">${h.reasons.map(esc).join(' · ')}</p><p class="hint">레이팅 ${esc(h.rating??'—')} · 부담 ${esc(h.burden??'—')}kg · 마체중 ${esc(h.horse_weight??'미발표')} · 데이터 ${Math.round(h.quality*100)}%</p></div>`).join('');$('#savePrediction').disabled=!r.pairs.length||manual||!Number.isFinite(start(r))||start(r)<=Date.now()||warning.length>0;renderOverview();}
