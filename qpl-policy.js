'use strict';
const Betkj3Policy=(()=>{
 const VERSION='betkj3-final-place-range-v2';
 ...
 function summarize(rows,range,from,to){
  const groups=Object.fromEntries(['all','seoul','busan','jeju'].map(k=>[k,{total:0,evaluated:0,hits:0,excluded:0}]));
  for(const row of rows){
   if(row.date<from||row.date>to)continue;
   const pick=row.settled?choose(row.candidates,range):null;
   for(const key of ['all',row.venue]){const g=groups[key];if(!g)continue;g.total++;if(pick){g.evaluated++;g.hits+=pick.hit?1:0;}else g.excluded++;}
  }
  return groups;
 }
 function apply(base,market,options){
  ...
  const anchor=ranked[0],partners=ranked.slice(range.min-1,range.max);
  ...
  result.qplPolicy={status:'ready',version:VERSION,baseModel:base.models.pair,anchor,partner:candidates[0].partner,ranked:ranked.slice(0,range.max),candidates,range};
  result.selection.pair.reason='연승 배당 1위 + 배당 '+range.min+'~'+range.max+'위 중 기존 모델 동반입상확률 우위 조합';
  return result;
 }
